...
async function api(request,env){
  ...
  if(path==='/api/health') return json({ok:true,version:'0.12.0',commerce:commerceReadiness(env),stripeTest:stripeTestReadiness(env),booksApp:booksAppCapabilities(env),mode:env.MARKETPLACE_MODE||'demo',authMode:env.YASREADY_AUTH_MODE||'demo',checkoutEnabled:env.CHECKOUT_ENABLED==='true',stripeMode:env.STRIPE_MODE||'off',ingramMode:env.INGRAM_MODE||'off',database:!!env.DB});
...
export default {
  async fetch(request,env){
    ...
    return new Response('Marketplace | YasReady · v0.12.0',{headers:{'content-type':'text/plain;charset=utf-8'}});
  }
};
