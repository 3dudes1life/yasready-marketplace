...
export function campaignUrl(book,campaign='direct',source='author-kit',medium='referral',campaignId=''){
  ...
  const url=isProjectPages?new URL(location.pathname,location.origin):new URL(`/book/${book.slug}`,location.origin);
  if(isProjectPages) url.searchParams.set('book',book.slug);
