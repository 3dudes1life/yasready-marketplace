...
function integrationsView(){return `<main class="view creatorView">${pageHead('Connections','One YasReady account. Clean product boundaries.','v0.7.0 keeps identity, Publishing handoff, payments, fulfillment and Business analytics inside one consistent YasReady operating system.')}
...
function render(){
  ...
  app.innerHTML=shell()+(state.loading?loadingState():(views[state.view]||storeView)())+`<footer><div class="shell"><div>${brandMark}<strong>Marketplace <span>|</span> YasReady</strong></div><p>Make it. Sell it. Understand it.</p><span>v0.7.0 · Catalog Management</span></div></footer>`+(state.selected?bookModal(state.selected):'')+(state.catalogEditor?catalogEditorModal():'');
...
function adaptApiBook(b){
  return {id:b.id,slug:b.slug,title:b.title,subtitle:b.subtitle,author:b.author?.name||state.account.name||'YasReady Author',authorId:b.author?.id||state.account.authorId,handle:b.author?.handle||state.account.handle,description:b.description,longDescription:b.longDescription,coverUrl:b.coverUrl||null,cover:b.coverUrl?`url(${b.coverUrl}) center/cover` : undefined,categories:[b.category].filter(Boolean),category:b.category,excerpt:b.excerpt||null,listingStatus:b.listing?.status||'draft',visibility:b.listing?.visibility||'public',listingRevision:Number(b.listing?.editorRevision??b.listing?.revision??0),seoTitle:b.listing?.seoTitle||null,seoDescription:b.listing?.seoDescription||null,launchAt:b.listing?.scheduledLiveAt||null,editions:(b.editions||[]).map(e=>({id:e.id,format:formatLabel(e.format),price:(e.priceMinor||0)/100,priceMinor:e.priceMinor,isbn:e.isbn,status:e.status,fulfillment:e.fulfillmentProvider,inventoryStatus:e.inventoryStatus,productionStatus:e.productionStatus}))};
}
...
async function hydrateFromApi(){
  try{
    const [catalogRes,sessionRes,myRes]=await Promise.all([fetch('/api/catalog',{headers:{accept:'application/json'}}),authFetch('/api/session',{headers:{accept:'application/json'}}),authFetch('/api/me/books',{headers:{accept:'application/json'}})]);
    if(!catalogRes.ok||!sessionRes.ok) return;
    const catalog=await catalogRes.json(),session=await sessionRes.json(),myPayload=myRes.ok?await myRes.json():{books:[]};
