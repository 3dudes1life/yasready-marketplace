...
function shell(){
  const nav=[['store','Marketplace'],['dashboard','Home'],['books','My Books'],['sales','Sales'],['marketing','Promote'],['integrations','Connections']];
  return `<div class="topChrome">
    ...
    <div class="modeStrip"><div class="shell"><span><strong>v0.2.0 · Marketplace Engine</strong> <i></i> ${state.api?'D1/API connected':'demo data'} <i></i> same YasReady account</span><span class="safePill">LIVE MONEY OFF</span></div></div>
  </div>`;
}
...
function booksView(){
  ...
  <div class="shell"><div class="noticeBar"><div class="noticeIcon">↔</div><div><strong>Shared-account contract is built into v0.2.0.</strong><span>`+'`authors.user_id`'+` maps to the same central YasReady identity. Marketplace never creates a second password.</span></div></div>
...
function integrationsView(){return `<main class="view creatorView">${pageHead('Connections','The plugs are built before we need them.','v0.2.0 separates identity, payments, fulfillment and Business analytics so none of those systems gets hard-wired into another product.')}
...
function cartDrawer(){
  ...
  return `<div class="drawerShade" data-close-cart><aside class="cartDrawer" onclick="event.stopPropagation()"><div class="drawerHead"><div><span class="microLabel">YOUR BAG</span><h2>${state.cart.length} ${state.cart.length===1?'item':'items'}</h2></div><button data-close-cart>×</button></div>${state.cart.length?`<div class="cartItems">${state.cart.map((x,i)=>`<div class="cartItem"><div><strong>${esc(x.title)}</strong><span>${esc(x.author)} · ${esc(x.format)}</span></div><div><strong>${money(x.price)}</strong><button data-remove="${i}">Remove</button></div></div>`).join('')}</div><div class="cartSummary"><div><span>Subtotal</span><strong>${money(total)}</strong></div><div><span>Independent authors</span><strong>${sellers}</strong></div></div><button class="btn primary wide" data-demo-checkout>Continue to checkout</button><p class="cartFine">v0.2 validates edition price and seller server-side before Stripe. Live checkout remains intentionally disabled in this package.</p>`:`<div class="emptyCart"><strong>Your bag is empty.</strong><p>Add an edition from any Marketplace listing.</p></div>`}</aside></div>`;
}

function render(){
  const views={store:storeView,dashboard:dashboardView,books:booksView,sales:salesView,marketing:marketingView,integrations:integrationsView};
  app.innerHTML=shell()+(views[state.view]||storeView)()+`<footer><div class="shell"><div>${brandMark}<strong>Marketplace <span>|</span> YasReady</strong></div><p>Make it. Sell it. Understand it.</p><span>v0.2.0 · private beta foundation</span></div></footer>`+(state.selected?bookModal(state.selected):'');
