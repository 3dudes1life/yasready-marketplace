...
function marketingView(){
  ...
  const rows=state.campaigns.filter(c=>!c.bookId||c.bookId===b.id).map(c=>({...c,visits:Number(c.visits??c.clicks??0),revenueMinor:Math.round(Number(c.revenue||0)*100),costMinor:Math.round(Number(c.cost||0)*100),label:channelConfig(c.source).label,...campaignMetrics({visits:c.visits??c.clicks,orders:c.orders,revenueMinor:Math.round(Number(c.revenue||0)*100),costMinor:Math.round(Number(c.cost||0)*100)})}));
  const total=rows.reduce((a,c)=>({visits:a.visits+c.visits,orders:a.orders+c.orders,revenueMinor:a.revenueMinor+c.netRevenueMinor,costMinor:a.costMinor+c.costMinor}),{visits:0,orders:0,revenueMinor:0,costMinor:0});
  const overall=campaignMetrics(total),recommendation=marketingRecommendation(rows);
  const currentUrl=campaignUrl(b,state.campaignName,state.campaignSource,state.campaignMedium),kit=launchKit({title:b.title,author:b.author,url:currentUrl,objective:state.campaignObjective});
  const top=[...rows].sort((a,z)=>z.netRevenueMinor-a.netRevenueMinor)[0];
  return `<main class="view creatorView">${pageHead('Marketing Studio','Help the right readers actually find the book.','Create the link, QR, embed and launch copy in one place — then let Marketplace connect the traffic and sales back to the campaign.',`<button class="btn secondary" data-nav="sales">Sales intelligence</button><button class="btn primary" data-scroll="campaign-builder">New campaign</button>`)}
  <div class="shell marketingStudio"><div class="promoteBookSelect"><label>Book</label><div class="selectWrap"><select id="marketingBook">${myBooks().map(x=>`<option value="${x.id}" ${x.id===b.id?'selected':''}>${esc(x.title)}</option>`).join('')}</select></div><span class="liveBadge">Marketing ready</span></div>
  <div class="metricsGrid five marketingMetrics">${metric('Tracked visits',total.visits.toLocaleString(),'Campaign landings','')}${metric('Attributed orders',total.orders,`${overall.conversionRate}% conversion`,'limeCard')}${metric('Attributed revenue',moneyMinor(total.revenueMinor),'After tracked refunds','')}${metric('Recorded spend',moneyMinor(total.costMinor),'Optional author-entered costs','')}${metric('Top channel',top?channelConfig(top.source).label:'No data',top?`${moneyMinor(top.netRevenueMinor)} attributed`:'Create a campaign','')}</div>
  <section class="marketingInsight"><div><span class="microLabel">YASREADY SIGNAL</span><h2>${esc(recommendation.title)}</h2><p>${esc(recommendation.detail)}</p></div><span class="insightBadge">Evidence-based</span></section>

  <div class="marketingGrid studioBuilder" id="campaign-builder"><section class="panel campaignBuilder"><div class="panelHead"><div><span class="microLabel">CAMPAIGN BUILDER</span><h2>One campaign. Every asset.</h2></div><span class="freeTag">FREE</span></div><div class="formGrid"><label>Campaign name<input id="campaignName" value="${esc(state.campaignName)}"></label><label>Goal<select id="campaignObjective"><option value="launch" ${state.campaignObjective==='launch'?'selected':''}>Book launch</option><option value="sales" ${state.campaignObjective==='sales'?'selected':''}>Drive sales</option><option value="awareness" ${state.campaignObjective==='awareness'?'selected':''}>Awareness</option><option value="reviews" ${state.campaignObjective==='reviews'?'selected':''}>Reader reviews</option><option value="event" ${state.campaignObjective==='event'?'selected':''}>Event</option><option value="evergreen" ${state.campaignObjective==='evergreen'?'selected':''}>Evergreen</option></select></label><label>Channel<select id="campaignSource">${[['instagram','Instagram'],['facebook','Facebook'],['tiktok','TikTok'],['youtube','YouTube'],['email','Email / newsletter'],['event-qr','Event / printed QR'],['author-site','Author website'],['press','Press / review'],['other','Other']].map(([v,l])=>`<option value="${v}" ${state.campaignSource===v?'selected':''}>${l}</option>`).join('')}</select></label><label>Planned spend <span class="optional">optional</span><input id="campaignBudget" type="number" min="0" step="0.01" value="${Number(state.campaignBudget||0).toFixed(2)}"></label></div><label class="fieldLabel">Trackable YasReady link</label><div class="copyField"><input id="campaignLink" readonly value="${esc(currentUrl)}"><button data-copy="#campaignLink">Copy</button></div><div class="assetPlan">${assetPlan({source:state.campaignSource,objective:state.campaignObjective}).map(a=>`<span>${esc(a.replaceAll('_',' '))}</span>`).join('')}</div><div class="campaignActions"><button class="btn primary" id="saveCampaign">Save campaign</button><button class="btn secondary" id="copyCampaignLink" data-copy="#campaignLink">Copy link</button></div></section>
  <section class="panel qrPanel"><div class="panelHead"><div><span class="microLabel">CAMPAIGN QR</span><h2>Offline can still be measurable.</h2></div><span class="freeTag">FREE</span></div><div class="qrFrame"><canvas id="qr"></canvas></div><p>This QR keeps the exact campaign ID, so an event, bookmark or package insert can earn attribution like a digital link.</p><button class="btn secondary" id="downloadQr">Download PNG</button></section></div>

  <section class="panel launchKitPanel"><div class="panelHead"><div><span class="microLabel">LAUNCH KIT</span><h2>Don’t make authors become marketers from scratch.</h2></div><span class="freeTag">FREE</span></div><div class="launchKitGrid"><div class="launchChecklist"><h3>${state.campaignObjective==='launch'?'Launch checklist':'Campaign checklist'}</h3>${kit.checklist.map((x,i)=>`<div><span>${i+1}</span><p>${esc(x)}</p></div>`).join('')}</div><div class="copyStack"><div class="copyOption"><span>Social launch</span><p id="socialLaunch">${esc(kit.social.launch)}</p><button data-copy="#socialLaunch">Copy</button></div><div class="copyOption"><span>Email subject</span><p id="emailSubject">${esc(kit.email.subject)}</p><button data-copy="#emailSubject">Copy</button></div><div class="copyOption"><span>Email body</span><p id="emailBody">${esc(kit.email.body)}</p><button data-copy="#emailBody">Copy</button></div><div class="copyOption"><span>Event sign</span><p id="eventCopy">${esc(`${kit.event.headline} — ${kit.event.subhead}`)}</p><button data-copy="#eventCopy">Copy</button></div></div></div></section>

  <div class="marketingGrid second"><section class="panel"><div class="panelHead"><div><span class="microLabel">WEBSITE TOOLKIT</span><h2>Turn the author’s site into a storefront.</h2></div><span class="freeTag">FREE</span></div><div class="embedPreview"><div class="tinyCover" style="background:${coverTheme(b)}"></div><div><strong>${esc(b.title)}</strong><span>${esc(b.author)}</span><button>See formats</button></div></div><pre id="embedCode">${esc(embedCode(b))}</pre><button class="btn secondary" data-copy-text="${encodeURIComponent(embedCode(b))}">Copy HTML button</button></section>
  <section class="panel"><div class="panelHead"><div><span class="microLabel">CHANNEL PLAYBOOK</span><h2>Use the right asset for the channel.</h2></div></div><div class="channelPlaybook"><div><b>Social</b><span>Short hook + tracked link</span></div><div><b>Email</b><span>Subject + reader CTA + tracked link</span></div><div><b>Events</b><span>Dedicated QR per event</span></div><div><b>Website</b><span>Persistent embed / buy button</span></div></div></section></div>

  <section class="panel campaignList marketingPerformance"><div class="panelHead"><div><span class="microLabel">CAMPAIGN PERFORMANCE</span><h2>Know what sold the book.</h2></div><span class="subtle">Revenue follows the campaign into checkout</span></div><div class="campaignTable studioTable"><div class="campaignHead"><span>Campaign</span><span>Channel</span><span>Visits</span><span>Orders</span><span>Conv.</span><span>Revenue</span><span>Spend</span><span>ROAS</span></div>${rows.map(c=>`<div class="campaignRow"><strong>${esc(c.name)}</strong><span>${esc(channelConfig(c.source).label)}</span><span>${c.visits}</span><span>${c.orders}</span><span>${c.conversionRate}%</span><span>${moneyMinor(c.netRevenueMinor)}</span><span>${c.costMinor?moneyMinor(c.costMinor):'Free'}</span><span>${c.roas==null?'—':`${c.roas}×`}</span></div>`).join('')}</div></section>
  <div class="freeToolkit"><span class="kicker">Marketing without another subscription</span><h2>The basics an indie author should have the minute the book goes live.</h2><div class="toolGrid">${['Permanent book URL','Unique campaign links','Campaign QR codes','Website embeds','Buy buttons','Launch checklist','Social copy','Email copy','Event copy','Attribution','Optional spend + ROAS','Business-ready marketing data'].map(x=>`<div><span>✓</span>${x}</div>`).join('')}</div></div>
  </div></main>`;
}
...
async function setupMarketing(){
  const select=document.querySelector('#marketingBook'); if(!select)return;
  select.onchange=()=>{state.marketingBookId=select.value;render()};
  const name=document.querySelector('#campaignName'),source=document.querySelector('#campaignSource'),objective=document.querySelector('#campaignObjective'),budget=document.querySelector('#campaignBudget'),link=document.querySelector('#campaignLink'),canvas=document.querySelector('#qr');
  const b=state.books.find(x=>x.id===state.marketingBookId)||myBooks()[0];
  async function update(){
    state.campaignName=name.value||'campaign';state.campaignSource=source.value;state.campaignObjective=objective.value;state.campaignBudget=Math.max(0,Number(budget.value)||0);
    state.campaignMedium=channelConfig(source.value).medium;const url=campaignUrl(b,state.campaignName,state.campaignSource,state.campaignMedium);link.value=url;await renderQr(canvas,url);
    const kit=launchKit({title:b.title,author:b.author,url,objective:state.campaignObjective});
    const values={socialLaunch:kit.social.launch,emailSubject:kit.email.subject,emailBody:kit.email.body,eventCopy:`${kit.event.headline} — ${kit.event.subhead}`};for(const [id,value] of Object.entries(values)){const el=document.querySelector('#'+id);if(el)el.textContent=value}
    track('marketing_asset_previewed',{type:'campaign_bundle',bookId:b.id,source:source.value,objective:objective.value});
  }
  name.oninput=update;source.onchange=()=>{update();render()};objective.onchange=()=>{update();render()};budget.oninput=()=>{state.campaignBudget=Math.max(0,Number(budget.value)||0)};await update();
  ...
  document.querySelector('#downloadQr').onclick=()=>{const a=document.createElement('a');a.download=`${b.slug}-${state.campaignName.toLowerCase().replace(/[^a-z0-9]+/g,'-')}-qr.png`;a.href=canvas.toDataURL('image/png');a.click();track('marketing_asset_downloaded',{type:'qr',bookId:b.id,campaign:state.campaignName});};
  document.querySelector('#saveCampaign').onclick=async()=>{
    const payload={bookId:b.id,name:state.campaignName,source:state.campaignSource,medium:state.campaignMedium,objective:state.campaignObjective,budgetMinor:Math.round(state.campaignBudget*100)};
    try{const r=await authFetch('/api/me/campaigns',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});if(!r.ok)throw new Error();const d=await r.json();state.campaigns.unshift({id:d.campaign.id,bookId:b.id,name:state.campaignName,source:state.campaignSource,objective:state.campaignObjective,clicks:0,visits:0,orders:0,revenue:0,cost:state.campaignBudget});toast('Campaign + marketing kit saved')}catch{state.campaigns.unshift({id:'local-'+Date.now(),bookId:b.id,name:state.campaignName,source:state.campaignSource,objective:state.campaignObjective,clicks:0,visits:0,orders:0,revenue:0,cost:state.campaignBudget});toast('Campaign saved in demo mode')}track('campaign_created',{bookId:b.id,source:state.campaignSource,objective:state.campaignObjective,budgetMinor:payload.budgetMinor});render();
  };
}
