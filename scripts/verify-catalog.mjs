...
const checks=[
 ...
 ['catalog version label is present',()=>assert.match(main,/v0\.10\.0 · Analytics Brain/)]
];
let passed=0;for(const [name,fn] of checks){try{fn();passed++;console.log(`PASS: ${name}`)}catch(e){console.error(`FAIL: ${name}`);throw e}}console.log(`PASS: ${passed}/${checks.length} v0.10 Catalog Management regression checks`);
